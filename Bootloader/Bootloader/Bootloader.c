...
static uint8_t BL_Host_Buffer[BL_HOST_BUFFER_RX_LENTH];
...
BL_Status BL_UART_Featch_Host_Command(void){
	...
	memset(BL_Host_Buffer, 0, BL_HOST_BUFFER_RX_LENTH );
...
void BL_Print_Message(char *Format, ...){
	...
	/* Enables Access to Variables Arguments */
	va_start(args,Format);
	
	/* Writes Formatted Data from the Variable Argument List to String */
	vsprintf(Message,Format,args);

#if (BL_DEBUG_METHOD == BL_ENABLE_UART_DEBUG_MESSAGE)
	/* Transmite the Formatted Data through The defined UART */
	...
	/* Transmite the Formatted Data through The defined SPI */
	
#elif (BL_DEBUG_METHOD == BL_ENABLE_CAN_DEBUG_MESSAGE)
	/* Transmite the Formatted Data through The defined CAN */	

#endif	
	
	/* Perform Cleanup for an ap Object intiallized by a call to va_start */
	va_end(args);
...
static void bootloader_jump_to_user_app(void){
	...
	// void (*pMainApp)(void) = (void *)MainAppAddr;
...
static uint8_t Bootloader_CRC_Verify(uint8_t *pData, uint32_t Data_Len, uint32_t Host_CRC){
	...
	/* Comapare the Host CRC and Calculate The CRC */
...
static void Bootloader_Get_Version(uint8_t *Host_Buffer){
	...
	/* Extract the CRC and Packet Length sent by the Host */
	Host_CMD_Packet_Len = Host_Buffer[0] + 1;
	Host_CRC32 = *(uint32_t *)(( Host_Buffer + Host_CMD_Packet_Len ) - CRC_TYPE_SIZE_BYTE) ;
	
	/*CRC Verification */
	if( Bootloader_CRC_Verify((uint8_t *)&Host_Buffer[0], Host_CMD_Packet_Len - 4, Host_CRC32) == CRC_VERIFICATION_PASSED){
...
static uint8_t Host_Jump_Address_Verification(uint32_t Jump_Address){
	...
	if((Jump_Address >= SRAM_BASE) && (Jump_Address <= STM32F103_SRAM_SIZE)){
		Address_Verification = ADDRESS_IS_VALID;
	}
	else if((Jump_Address >= FLASH_BASE) && (Jump_Address <= STM32F103_FLASH_SIZE)){
		Address_Verification = ADDRESS_IS_VALID;
	}	
	else{
...
static void Bootloader_Jump_To_Address(uint8_t *Host_Buffer){
	...
	if( Bootloader_CRC_Verify((uint8_t *)&Host_Buffer[0], Host_CMD_Packet_Len - 4, Host_CRC32) == CRC_VERIFICATION_PASSED){
		...
		/* Extract Address from the Host Packet */
		HOST_Jump_Address = *((uint32_t *)&Host_Buffer[2]);
		
		/* Verify The Extract Address to be Valid Address */
		Address_Verification = Host_Jump_Address_Verification(HOST_Jump_Address);
		
		if (Address_Verification == ADDRESS_IS_VALID){
	#if (BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE)
		BL_Print_Message(" Address Verification Succsedd \r\n");
	#endif	
			
			/* Report Address Verification Succsedd */
		...
		/* Report Address_Verification Succsedd */
		Bootloader_Send_Data_To_Host((uint8_t *)&Address_Verification, 1);
...
#if (BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE)
                BL_Print_Message("Flash Mass Erase Requested\r\n");
#endif
            }
            else
            {
                // Page erase
#if (BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE)
                BL_Print_Message("Flash Page Erase Requested\r\n");
#endif
                ...
                pEraseInit.PageAddress = FLASH_BASE + (Page_Number * 1024); // 1 KB per page
...
static HAL_StatusTypeDef HAL_FLASH_OB_Launch_Status(void){
    HAL_FLASH_OB_Launch();

    // Check if an operation error occurred
...
static uint8_t Change_ROP_Level(uint32_t ROP_Level){
	...
	BL_Print_Message("Failed -> Unlock the FLASH Option Control Registers access \r\n");
		...
		if(HAL_Status != HAL_OK){
			...
			HAL_Status = HAL_FLASH_OB_Launch_Status();
			if(HAL_Status != HAL_OK){
				ROP_Level_Status = ROP_LEVEL_CHANGE_INVALID;
			}
			else{
